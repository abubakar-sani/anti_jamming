...
jammerType = 'complete'
network = 'FNN'
cscs = [0, 0.1, 0.2, 0.3, 0.4]  # Channel switching costs
...
for csc in cscs:
    ...
    DDQN_agent = DoubleDeepQNetwork(s_size, a_size, history, lr, discount_rate, epsilon, epsilon_min, epsilon_decay)
    rewards = []  # Store rewards for graphing
    epsilons = []  # Store the Explore/Exploit

    # Training agent
    for e in range(TRAIN_Episodes):
        # Lines for implementing FIFO
        historical_state = np.zeros((history, s_size), dtype='float32')
        next_historical_state = historical_state
        state = env.reset()
        # print(f"Initial state is: {state}")
        state = np.reshape(state, [1, s_size])  # Resize to store in memory to pass to .predict
        historical_state[1:-1] = historical_state[:-2]
        historical_state[0] = state
        tot_rewards = 0
        previous_action = 0
        for time in range(max_env_steps):
            reshaped_historical_state = historical_state.reshape((1, history * s_size))
            action = DDQN_agent.action(reshaped_historical_state)
            ...
            next_historical_state = historical_state
            next_historical_state[1:-1] = historical_state[:-2]
            next_historical_state[0] = next_state
            reshaped_next_historical_state = next_historical_state.reshape((1, history * s_size))
            tot_rewards += reward
            DDQN_agent.store(reshaped_historical_state, action, reward, reshaped_next_historical_state, done)  # Resize to store in memory to pass to .predict
            historical_state = next_historical_state
    ...
    for e_test in range(TEST_Episodes):
        ...
        for t_test in range(max_env_steps):
            reshaped_historical_state = historical_state.reshape((1, history * s_size))
            action = DDQN_agent.test_action(reshaped_historical_state)
            next_state, reward, done, _ = env.step(action)
            if done or t_test == max_env_steps - 1:
                ...
                break
            next_state = np.reshape(next_state, [1, s_size])
            # Converting next_state to historical form
            next_historical_state = historical_state
            next_historical_state[1:-1] = historical_state[:-2]
            next_historical_state[0] = next_state
            # Flattening next historical state
            reshaped_next_historical_state = next_historical_state.reshape((1, history * s_size))
            # Cumulating the reward
            tot_rewards += reward
    ...
    # Plotting
    plotName = f'results/{network}/M={history}/{jammerType}_csc_{csc}.png'
    ...
    # Save Results
    # Rewards
    fileName = f'results/{network}/M={history}/rewards_{jammerType}_csc_{csc}.json'
    ...
    fileName = f'results/{network}/M={history}/throughput_{jammerType}_csc_{csc}.json'
    with open(fileName, 'w') as f:
    ...
    fileName = f'results/{network}/M={history}/times_{jammerType}_csc_{csc}.json'
    with open(fileName, 'w') as f:
        json.dump(normalized_cst, f)
    # Save the agent as a SavedAgent.
    agentName = f'savedAgents/{network}/M={history}/DDQNAgent_{jammerType}_csc_{csc}'
