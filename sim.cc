...
NS_LOG_COMPONENT_DEFINE ("jammer-Pattern");
...
int main (int argc, char *argv[])
{
  ...
  // Jammer Pattern
  double jammerSlotTime = 0.1;  // seconds;
  ...
  //        time,    channel usage
    ...
    double strategy = 1;
    double jammer;
    std::map <uint32_t, std::vector<uint32_t>> jammerPattern;

    // Random jamming strategy
    if (strategy == 1) {
        jammer = rand() % 3 + 1;
    } else {
        jammer = 2;
    ...
    // Combined jammer
    if (jammer == 1) {
        jammerPattern.insert(std::pair < uint32_t, std::vector < uint32_t > > (0, {1, 0, 1, 0, 1}));
    }
        // Sweeping jammer
    else if (jammer == 2) {
        jammerPattern.insert(std::pair < uint32_t, std::vector < uint32_t > > (0, {1, 0, 0, 0, 0}));
        jammerPattern.insert(std::pair < uint32_t, std::vector < uint32_t > > (1, {0, 1, 0, 0, 0}));
        jammerPattern.insert(std::pair < uint32_t, std::vector < uint32_t > > (2, {0, 0, 1, 0, 0}));
        jammerPattern.insert(std::pair < uint32_t, std::vector < uint32_t > > (3, {0, 0, 0, 1, 0}));
        jammerPattern.insert(std::pair < uint32_t, std::vector < uint32_t > > (4, {0, 0, 0, 0, 1}));
    }
        // Random channel hopping jammer
    else {
        uint32_t unique_patterns = simulationTime/jammerSlotTime+2;
        for (uint32_t patternId = 0; patternId < unique_patterns; patternId++) {
            // Defining patterns
            uint32_t pattern = rand() % channNum;
            switch (pattern) {
                case 0  :
                    jammerPattern.insert(std::pair < uint32_t, std::vector < uint32_t > > (patternId, {1, 0, 0, 0, 0}));
                    break;
                case 1  :
                    jammerPattern.insert(std::pair < uint32_t, std::vector < uint32_t > > (patternId, {0, 1, 0, 0, 0}));
                    break;
                case 2  :
                    jammerPattern.insert(std::pair < uint32_t, std::vector < uint32_t > > (patternId, {0, 0, 1, 0, 0}));
                    break;
                case 3  :
                    jammerPattern.insert(std::pair < uint32_t, std::vector < uint32_t > > (patternId, {0, 0, 0, 1, 0}));
                    break;
                case 4  :
                    jammerPattern.insert(std::pair < uint32_t, std::vector < uint32_t > > (patternId, {0, 0, 0, 0, 1}));
                    break;
            }
        }
    }
  ...
  // Signal Generator --- Generate jammer pattern
  Ptr<Node> interferingNode = nodes.Get(1);
  WaveformGeneratorHelper waveformGeneratorHelper;
  waveformGeneratorHelper.SetChannel (spectrumChannel);
  waveformGeneratorHelper.SetPhyAttribute ("Period", TimeValue (Seconds (jammerSlotTime)));
